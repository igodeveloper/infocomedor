...
class Application_Model_DataService {
     ...
     * @copyright Conexion S.A.
     * @author Andre L'Heureux
     */
...
//die();
     ...
     * @author Andre L'Heureux
     */
    public function getRowsBySelect($fields, $where = null, $order = null, $group = null) {
...
//                        die($clave);
            if (count($clave) == 0) {
                //INSERT
                //die("entroooo por aca 1");
                $clave1 = $this->insertRow($hayClave, $row);
            } else {
                //ECHO("saveRow entre por aca 1<br>");                            
//				print_r($clave); 				 die();
                //UPDATE
                //Buscar Registro
                $rowExsite = null;
                if ($hayClave) {
//                    ECHO("hay clave saveRow entre por aca 1<br>");
...
//                        ECHO("entre por aca 1<br>");
                    } else {


                        $clave1 = $laTabla->update($row->toArr(), $where);
//                        ECHO("entre por aca 2<br>");
                    }
                } else {
//                    ECHO("entre por aca 3<br>");
//                                 print_r($where);
//                                            die();   
                    $clave1 = $this->insertRow($hayClave, $row);
                }
