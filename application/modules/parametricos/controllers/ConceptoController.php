...
class Parametricos_ConceptoController extends Zend_Controller_Action
{
    ...
    {
//        $table = new Zend_ModelCreator('tipo_producto','infocomedor');
        $parametrosNamespace = new Zend_Session_Namespace ( 'parametros' );
        $parametrosNamespace->unlock ();
        $parametrosNamespace->parametrosBusqueda = null;
        $parametrosNamespace->cantidadFilas = null;
        $parametrosNamespace->jsGrip = '/js/grillasmodulos/parametricos/gridConcepto.js';
        $parametrosNamespace->Application_Model_DbTable = "Application_Model_DbTable_Concepto";
        $parametrosNamespace->busqueda = "DS_CONCEPTO";
	...
	private function obtenerPaginas($result,$cantidadFilas,$page){
		...
		foreach ( $this->_paginator as $item ) {
			$arrayDatos ['cell'] = array($item["COD_CONCEPTO"],null,trim(utf8_encode($item["DS_CONCEPTO"])),$item["CONCEPTO_ACCION"]);
			$arrayDatos ['columns'] = array("id","modificar","nombre","accion","");
    ...
    public function eliminarAction(){
		...
		try{
                    ...
                    $db->beginTransaction();
                    $servCon = new Application_Model_DataService($parametrosNamespace->Application_Model_DbTable);
                    $servCon->deleteRowById(array("COD_CONCEPTO"=>$id));
                    $db->commit();
	...
	public function guardarAction(){
            ...
            $rowData = json_decode($json_rowData);
            $applicationModel = new Application_Model_Concepto();
	...
	public function modificarAction(){
            ...
            $json_rowData = $this->getRequest ()->getParam ( "parametros" );
            $rowData = json_decode($json_rowData);
            $rowClass = new Application_Model_Concepto();
		if($rowData->idRegistro != null){
				$rowClass->setCod_Concepto($rowData->idRegistro);
			}
        ...
        public function almacenardatos($rowClass,$rowData){
     	try{
        	...
        	$db = Zend_Db_Table::getDefaultAdapter();
        	$db->beginTransaction();
                $rowClass->setDesc_Concepto(trim(utf8_decode($rowData->descripcionConcepto)));
                $rowClass->setAccion_Concepto($rowData->accionConcepto);
