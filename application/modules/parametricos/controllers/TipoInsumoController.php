...
class Parametricos_TipoInsumoController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        $parametrosNamespace->jsGrip = '/js/grillasmodulos/parametricos/gridTipoInsumo.js';
        $parametrosNamespace->Application_Model_DbTable = "Application_Model_DbTable_TipoInsumo";
        $parametrosNamespace->busqueda = "TIPO_INSUMO_DESCRIPCION";
	...
	private function obtenerPaginas($result,$cantidadFilas,$page){
		...
		foreach ( $this->_paginator as $item ) {
			$arrayDatos ['cell'] = array($item["COD_TIPO_INSUMO"],null,trim(utf8_encode($item["TIPO_INSUMO_DESCRIPCION"])),$item["TIPO_INSUMO_ISO"]);
			$arrayDatos ['columns'] = array("id","modificar","nombre","iso");
    ...
    public function eliminarAction(){
		...
		try{
                    ...
                    $db->beginTransaction();
                    $servCon = new Application_Model_DataService($parametrosNamespace->Application_Model_DbTable);
                    $servCon->deleteRowById(array("COD_TIPO_INSUMO"=>$id));
                    $db->commit();
	...
	public function guardarAction(){
            ...
            $rowData = json_decode($json_rowData);
            $applicationModel = new Application_Model_TipoInsumo();
	...
	public function modificarAction(){
            ...
            $json_rowData = $this->getRequest ()->getParam ( "parametros" );
            $rowData = json_decode($json_rowData);
            $rowClass = new Application_Model_TipoInsumo();
		if($rowData->idRegistro != null){
				$rowClass->setCod_tipo_insumo($rowData->idRegistro);
			}
        ...
        public function almacenardatos($rowClass,$rowData){
     	try{
        	...
        	$db = Zend_Db_Table::getDefaultAdapter();
        	$db->beginTransaction();
                $rowClass->setDesc_tipo_insumo(trim(utf8_decode($rowData->descripcionTipoInsumo)));
                $rowClass->setIso_tipo_insumo($rowData->isoTipoInsumo);
