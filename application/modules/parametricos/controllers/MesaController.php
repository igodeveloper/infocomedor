...
class Parametricos_MesaController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        $parametrosNamespace->jsGrip = '/js/grillasmodulos/parametricos/gridMesa.js';
        $parametrosNamespace->Application_Model_DbTable = "Application_Model_DbTable_Mesa";
        $parametrosNamespace->busqueda = "DES_MESA";
        $parametrosNamespace->lock ();
    }
	...
	private function obtenerPaginas($result,$cantidadFilas,$page){
		...
		foreach ( $this->_paginator as $item ) {
			$arrayDatos ['cell'] = array($item["COD_MESA"],null,$item["COD_MESA"],trim(utf8_encode($item["DES_MESA"])));
			$arrayDatos ['columns'] = array("id","modificar","codigo","nombre");
    ...
    public function eliminarAction(){
		...
		try{
                    ...
                    $db->beginTransaction();
                    $servCon = new Application_Model_DataService($parametrosNamespace->Application_Model_DbTable);
                    $servCon->deleteRowById(array("COD_MESA"=>$id));
                    $db->commit();
	...
	public function guardarAction(){
            ...
            $rowData = json_decode($json_rowData);
            $applicationModel = new Application_Model_Mesa();
	...
	public function modificarAction(){
            ...
            $json_rowData = $this->getRequest ()->getParam ( "parametros" );
            $rowData = json_decode($json_rowData);
            $rowClass = new Application_Model_Mesa();
		if($rowData->idRegistro != null){
				$rowClass->setCod_mesa($rowData->idRegistro);
			}
        ...
        public function almacenardatos($rowClass,$rowData){
     	try{
        	...
        	$db = Zend_Db_Table::getDefaultAdapter();
        	$db->beginTransaction();
                $rowClass->setDesc_mesa(trim(utf8_decode($rowData->descripcionMesa)));
