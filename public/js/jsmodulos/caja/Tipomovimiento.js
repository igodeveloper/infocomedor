...
$().ready(function() {
        $('#descripciontipomovimiento-filtro').attr("value",null);
	...
	$("#nuevoregistro").click(function() {
		$('#modalEditar').show();
                //ID de registro
		$('#codigotipomovimiento-modal').attr("value",null);
...
function enviarParametrosRegistro(data){
	...
	var urlenvio = '';
	if(data.cod_tipo_mov !== null && data.cod_tipo_mov.length !== 0){
...
function obtenerJsonFormulario() {
	...
	var mensaje = '';
        var error = 0;
	if($('#descripciontipomovimiento-modal').attr("value") == null || $('#descripciontipomovimiento-modal').attr("value").length == 0){
            mensaje+= ' | Ingrese la descripci\u00F3n | ';
            $('#descripciontipomovimiento-modal').attr("required", "required");
            error = 1;
	}
        if($('#operaciontipomovimiento-modal').attr("value") == -1 || $('#operaciontipomovimiento-modal').attr("value").length == 0){
            mensaje+= ' | Seleccione una operaci\u00F3n | ';
            $('#operaciontipomovimiento-modal').attr("required", "required");
            error = 1;
            //addrequiredattr('descripciontipomovimiento-modal',1);    
        }
        if(error == 1){
            ...
            jsonObject.cod_tipo_mov = $('#codigotipomovimiento-modal').attr("value");
            jsonObject.desc_tipo_mov = $('#descripciontipomovimiento-modal').attr("value");
            jsonObject.tipo_mov = $('#operaciontipomovimiento-modal').attr("value");
            return jsonObject;
	}
...
function editarRegistro(parametros){
	...
	$("#codigotipomovimiento-modal").attr("value",parametros.cod_tipo_mov);
	$("#descripciontipomovimiento-modal").attr("value",parametros.desc_tipo_mov);
	$("#operaciontipomovimiento-modal option[value="+parametros.tipo_mov+"]").attr("selected",true);
...
function limpiarFormulario(){
	...
	$("#success-block").hide();
	$("#codigotipomovimiento-modal").attr("value",null);
	$("#descripciontipomovimiento-modal").attr("value",null);
	$("#operaciontipomovimiento-modal").attr("value",-1);
}
...
function obtenerJsonBuscar(){
	...
	if($('#descripciontipomovimiento-filtro').attr("value") != null && $('#descripciontipomovimiento-filtro').attr("value").length != 0){
		jsonObject.descripcion = $('#descripciontipomovimiento-filtro').attr("value");
	}
