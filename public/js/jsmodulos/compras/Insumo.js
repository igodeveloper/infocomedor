...
mensajeWarning = new Array("Complete una Descripcion por favor","Seleccione el tipo de insumo por favor");
idCamposGrilla = new Array("id-registro","descripcionBusqueda-modal","comboTipoInsumo-modal");
...
$().ready(function() {
	...
	$("#nuevoregistro").click(function() {
		...
		$('#'+idCamposGrilla[0]).attr("value",null);
                $("#comboTipoInsumo-modal option[value=-1]").attr("selected",true);
...
function obtenerJsonFormulario() {
	...
	if($('#'+idCamposGrilla[1]).attr("value") == null || $('#'+idCamposGrilla[1]).attr("value").length == 0){
		...
		jsonObject.idRegistro = $('#'+idCamposGrilla[0]).attr("value");
                ...
                jsonObject.tipoInsumo = $('#'+idCamposGrilla[2]).attr("value");
		return jsonObject;
	}
    return null;
}
...
function editarRegistro(parametros){
	...
	$("#"+idCamposGrilla[1]).attr("value",parametros.descripcion);        
...
function obtenerJsonBuscar(){
	var jsonObject = new Object();
        ...
        jsonObject.comboTipoInsumo = null;
	...
	if($('#comboTipoInsumo').attr("value") != null && $('#comboTipoInsumo').attr("value").length != 0){
		jsonObject.comboTipoInsumo = $('#comboTipoInsumo').attr("value");
	}        
...
function cargarComboTipoInsumo(){
	...
	$.ajax({
        url: table+'/tipoinsumodata',
        type: 'post',
        dataType: 'html',
        async : false,
        success: function(respuesta){
        	$("#comboTipoInsumo").html(respuesta);
        	$("#comboTipoInsumo-modal").html(respuesta);
        },
        error: function(event, request, settings){
         //   $.unblockUI();
            alert("Ha ocurrido un error");
