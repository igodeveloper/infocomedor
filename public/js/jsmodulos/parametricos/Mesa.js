table = "/parametricos/Mesa/";
mensajeWarning = new Array("Complete una Descripcion por favor");
//aqui tiene que ir lo mismo que va en index.phtml
idCamposGrilla = new Array("id-registro","descripcionBusqueda-modal");
...
function obtenerJsonFormulario() {
	...
	/*} else if($('#'+idCamposGrilla[2]).attr("value") == null || $('#'+idCamposGrilla[2]).attr("value").length == 0){
		mostarVentana("warning-registro",mensajeWarning[1]);*/
            ...
            //LOS CAMPOS DEBEN LLAMARSE IGUAL QUE EN gridMesa.js
		jsonObject.idRegistro = $('#'+idCamposGrilla[0]).attr("value");
                jsonObject.descripcionMesa = $('#'+idCamposGrilla[1]).attr("value");
...
function editarRegistro(parametros){
	...
	$("#"+idCamposGrilla[1]).attr("value",parametros.descripcionMesa);
