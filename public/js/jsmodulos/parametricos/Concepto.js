//table = "/parametricos/Concepto/";
var pathname = window.location.pathname;
var table = pathname;
mensajeWarning = new Array("Complete una Descripcion por favor","Complete la accion E/S por favor");
//aqui tiene que ir lo mismo que va en index.phtml
idCamposGrilla = new Array("id-registro","descripcionBusqueda-modal","accionConcepto-modal");
...
function obtenerJsonFormulario() {
	...
	if($('#'+idCamposGrilla[1]).attr("value") == null || $('#'+idCamposGrilla[1]).attr("value").length == 0){
		...
		mostarVentana("warning-registro",mensajeWarning[1]);
            ...
            //LOS CAMPOS DEBEN LLAMARSE IGUAL QUE EN gridUnidadMedida.js
		jsonObject.idRegistro = $('#'+idCamposGrilla[0]).attr("value");
                jsonObject.descripcionConcepto= $('#'+idCamposGrilla[1]).attr("value");
                jsonObject.accionConcepto= $('#'+idCamposGrilla[2]).attr("value");
		return jsonObject;
	}
    return null;
}
...
function validarNumerosCampo(){
	 $("#"+idCamposGrilla[2]).keydown(function(event) {
...
function editarRegistro(parametros){
	...
	$("#"+idCamposGrilla[1]).attr("value",parametros.descripcionConcepto);
	$("#"+idCamposGrilla[2]).attr("value",parametros.accionConcepto);
