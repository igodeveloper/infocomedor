//table = "/parametricos/impuesto/";
var pathname = window.location.pathname;
var table = pathname;
mensajeWarning = new Array("Complete una Descripcion por favor","Complete la sigla por favor","Complete el porcentaje por favod");
//aqui tiene que ir lo mismo que va en index.phtml
idCamposGrilla = new Array("id-registro","descripcionImpuestoBusqueda-modal","siglaImpuesto-modal","porcentajeImpuesto-modal");
...
function obtenerJsonFormulario() {
	...
	if($('#'+idCamposGrilla[1]).attr("value") == null || $('#'+idCamposGrilla[1]).attr("value").length == 0){
		...
		mostarVentana("warning-registro",mensajeWarning[3]);
            ...
            //LOS CAMPOS DEBEN LLAMARSE IGUAL QUE EN gridImpuesto.js
		jsonObject.idRegistro = $('#'+idCamposGrilla[0]).attr("value");
                jsonObject.descripcionImpuesto = $('#'+idCamposGrilla[1]).attr("value");
                jsonObject.siglaImpuesto = $('#'+idCamposGrilla[2]).attr("value");
                jsonObject.porcentajeImpuesto = $('#'+idCamposGrilla[3]).attr("value");
		return jsonObject;
	}
    return null;
}
...
function editarRegistro(parametros){
	...
	$("#"+idCamposGrilla[1]).attr("value",parametros.descripcionImpuesto);
	$("#"+idCamposGrilla[2]).attr("value",parametros.siglaImpuesto);
        $("#"+idCamposGrilla[3]).attr("value",parametros.porcentajeImpuesto);
