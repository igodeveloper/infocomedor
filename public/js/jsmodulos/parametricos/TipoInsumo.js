...
idCamposGrilla = new Array("id-registro","descripcionBusqueda-modal","isotipoinsumo-modal");
...
function obtenerJsonFormulario() {
	...
	if($('#'+idCamposGrilla[1]).attr("value") == null || $('#'+idCamposGrilla[1]).attr("value").length == 0){
		...
		jsonObject.idRegistro = $('#'+idCamposGrilla[0]).attr("value");
                jsonObject.descripcionTipoInsumo = $('#'+idCamposGrilla[1]).attr("value");
                jsonObject.isoTipoInsumo = $('#'+idCamposGrilla[2]).attr("value");
		return jsonObject;
	}
    return null;
}
...
function validarNumerosCampo(){
	 $("#"+idCamposGrilla[0]).keydown(function(event) {
...
function editarRegistro(parametros){
	...
	$("#"+idCamposGrilla[1]).attr("value",parametros.descripcionTipoInsumo);
	$("#"+idCamposGrilla[2]).attr("value",parametros.isoTipoInsumo);
