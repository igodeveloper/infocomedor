...
mensajeWarning = new Array("Complete una Descripcion por favor","Complete el simbolo ISO por favor");
//aqui tiene que ir lo mismo que va en index.phtml
idCamposGrilla = new Array("id-registro","descripcionBusqueda-modal","isounidadmedida-modal");
...
function obtenerJsonFormulario() {
	...
	if($('#'+idCamposGrilla[1]).attr("value") == null || $('#'+idCamposGrilla[1]).attr("value").length == 0){
		...
		jsonObject.idRegistro = $('#'+idCamposGrilla[0]).attr("value");
                jsonObject.descripcionUnidadMedida = $('#'+idCamposGrilla[1]).attr("value");
                jsonObject.isoUnidadMedida = $('#'+idCamposGrilla[2]).attr("value");						
		return jsonObject;
	}
    return null;
}
...
function editarRegistro(parametros){
	...
	$("#"+idCamposGrilla[1]).attr("value",parametros.descripcionUnidadMedida);
	$("#"+idCamposGrilla[2]).attr("value",parametros.isoUnidadMedida);
