...
var table = pathname;
mensajeWarning = new Array("Complete una Descripci�n por favor","Complete la sigla por favor");
...
function obtenerJsonFormulario() {
	...
	if($('#'+idCamposGrilla[1]).attr("value") == null || $('#'+idCamposGrilla[1]).attr("value").length == 0){
		...
		jsonObject.idRegistro = $('#'+idCamposGrilla[0]).attr("value");
                jsonObject.descripcionestadopedido = $('#'+idCamposGrilla[1]).attr("value");
                jsonObject.siglaestadopedido = $('#'+idCamposGrilla[2]).attr("value");
		return jsonObject;
	}
    return null;
}
...
function editarRegistro(parametros){
	...
	$("#"+idCamposGrilla[0]).attr("value",parametros.idRegistro);
        $("#"+idCamposGrilla[1]).attr("value",parametros.descripcionestadopedido);
	$("#"+idCamposGrilla[2]).attr("value",parametros.siglaestadopedido);
